...
def train2():
    ...
    # Model definition (assuming the rest of the model architecture remains the same)
    input_layer = Input(shape=(X_train_scaled.shape[1],))

    # Common layers
    common_dense = Dense(256, activation='relu')(input_layer)
    common_dense = BatchNormalization()(common_dense)
    common_dense = Dropout(0.3)(common_dense)
    common_dense = Dense(128, activation='relu')(common_dense)
    common_dense = BatchNormalization()(common_dense)
    common_dense = Dropout(0.3)(common_dense)
    common_dense = Dense(64, activation='relu')(common_dense)

    output_layer = Dense(len(move_list), activation='softmax')(common_dense)
